...
def build_spectrogram_datasets(filelist):
    ...
    for index, file_path in enumerate(filelist):
        ...
        # matplotlib to create spectrogram
        sample_rate, samples = read(source_file)
        plt.specgram(samples, cmap='gist_rainbow', Fs=sample_rate)
        ...
        image_arr = image_arr[59:426, 82:575, :]
    ...
    np.save("./spectrogram_100x160_matplot_datasets/gtzan_spect_train_images.npy", train_data)
    np.save("./spectrogram_100x160_matplot_datasets/gtzan_spect_train_labels.npy", train_labels)
    np.save("./spectrogram_100x160_matplot_datasets/gtzan_spect_test_images.npy", test_data)
    np.save("./spectrogram_100x160_matplot_datasets/gtzan_spect_test_labels.npy", test_labels)
...
def main():
    ...
    try:
        shutil.rmtree("spectrogram_100x160_matplot_datasets")
    except FileNotFoundError:
        os.mkdir("spectrogram_100x160_matplot_datasets")
    else:
        os.mkdir("spectrogram_100x160_matplot_datasets")
