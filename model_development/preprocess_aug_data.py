# Author: Ian Docherty
# Description: This script takes the augmented filelist containing the paths of
#              the augmented data, then saves a numpy dataset to be used by the machine
#              learning models. Since there is a lot of data, only every 50th data
#              point in the WAV file is kept to improve model performance.
...
def build_numpy_dataset(file_list, train_or_test):
    ...
    # Save to numpy files
    np.save("./aug_split_datasets/gtzan_aug_split_" + train_or_test + "_audio.npy", dataset)
    np.save("./aug_split_datasets/gtzan_aug_split_" + train_or_test + "_labels.npy", labels)
...
if __name__ == "__main__":
    ...
    try:
        shutil.rmtree("aug_split_datasets")
    except FileNotFoundError:
        os.mkdir("aug_split_datasets")
    else:
        os.mkdir("aug_split_datasets")
